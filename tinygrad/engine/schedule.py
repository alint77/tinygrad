...
def create_schedule_with_vars(big_sink:UOp) -> tuple[list[ScheduleItem], dict[Variable, int], dict[UOp, UOp]]:
  ...
  for k,v in tensor_map.items():
    # NOTE: tensors can also map to a VIEW, we just apply the VIEW on top of buffer
    if (a:=kernel_map.get(v.base)) is not None and a.op is Ops.ASSIGN: becomes_map[k] = a.src[0] if v is v.base else a.src[0].view(unwrap(v.st))
    if v is k: continue
